...
export function Button({
  ...
  const StartIcon = iconPosition === "start" ? null : null;
  const EndIcon = iconPosition === "end" ? null : null;
  const classList = [];
  if (iconColor) {
  ...
  return (
    <button data-component="Button" className={classList.join(" ")} {...rest}>
      {StartIcon && (
        <span className="icon icon-start">{"Start" /* <StartIcon /> */}</span>
      )}
      <span className="label">{label}</span>
      {EndIcon && (
        <span className="icon icon-end">{"End" /* <EndIcon /> */}</span>
      )}
    </button>
  );
}
